...
const Messenger: React.FC = () => {
  ...
  return (
    <div className="w-full">
      {/* Top label */}
      <div
        className={`w-full sticky z-40 top-[2px] left-0 p-2 flex items-start justify-between ${
          isDark ? 'bg-darkBg text-darkText' : 'bg-lightBg text-left'
      ...
      >
        <div className="flex items-start justify-center gap-2">
          ...
          <div className="text-xs">
            <h3>Muhammed Musa</h3>
            <p className={`text-gray-500 mt-2 ${isDark ? 'text-white' : ''}`}>last seen 2h ago</p>
          </div>
        </div>

        {/* Right */}
        <div className="flex items-center justify-end gap-5">
          <IoCall className={`text-sm cursor-pointer ${isDark ? 'text-white' : 'text-gray-500'}`} />
          <FaVideo className={`text-sm cursor-pointer ${isDark ? 'text-white' : 'text-gray-500'}`} />
        </div>
      </div>

      {/* Chat section */}
      <div className="text-xs pt-5 pb-10">
        {sampleChat.map((eachChat, index) =>
          eachChat.senderId === 1 ? (
            <MainMessageText key={index} Status={Status1} content={eachChat.text} />
          ) : (
            <OtherMessageText key={index} Status={Status2} content={eachChat.text} />
      ...
      {/* Input section */}
      <div
        className={`w-full sticky bottom-10 left-0 flex items-center p-2 justify-between ${
          isDark ? 'bg-darkBg text-darkText' : 'bg-lightBg text-left'
        } transition-all duration-500`}
      >
        <input
          className="w-full px-2 py-3 text-black rounded-full outline-none"
          type="text"
          placeholder="Text"
        />
