  ...
  return (
    ...
    >
        ...
        <div className="absolute -bottom-8 left-0 flex items-start justify-start gap-5">
          ...
          <div>
            ...
            >
              ...
              <Link
                to={`/update/${auth._id}?id=${auth._id}&email=${auth.email}&step=stepOne`}
                className={`text-sm px-5 py-2 rounded-full ${
                  isDark ? "bg-lightBg text-deepBg" : "bg-deepBg text-lightText"
                }`}
              >
                update
              </Link>
      ...
      <section
        ...
        className={`mt-20 relative p-4 w-full carousel bg-transparent ${
      ...
      >
        ...
        <button
          onClick={scrollLeft}
          className="sticky top-1/3 -left-4 z-10 p-2 btn btn-circle"
        >
          <FaChevronLeft />
        </button>
        ...
        <button
          onClick={scrollRight}
          className="sticky right-0 top-1/3 z-10 p-2 btn btn-circle"
        >
          <FaChevronRight />
        </button>
      </section>
        ...
        <div>
          {activeSelection === "post" &&
            userPost?.map((post: any) => {
              return <Feed showAll={false} isDark={isDark} data={post} />;
            })}
          {activeSelection === "media" && (
            <div className="p-10 text-white">Media here</div>
          )}
          {activeSelection === "comments" && (
            <div className="p-10 text-white">Comments here</div>
          )}
        </div>
      </section>
    </div>
