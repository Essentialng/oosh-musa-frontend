...
const SidebarWeb = () => {
  ...
  return (
    <div className="mb-10">
      <ul
        className={`w-full sm:px-4 py-2 flex sm:flex-col flex-row items-start sm:justify-start justify-between gap-5 z-30`}
      >
        <ListLink
          Title="Home"
          Logo={
            <FaHome
              className={`w-3 h-3 sm:group-hover:w-4 sm:group-hover:h-4 transition-all duration-500`}
            />
          }
          link=""
        />
      ...
      <ul
        className={`w-full sm:px-4 py-2 flex sm:flex-col flex-row items-start sm:justify-start justify-between gap-5 z-50`}
      >
        ...
        {userId ? (
          <ChannelList link="earn" title="Earning" logo={<PiWallet />} />
        ) : null}
        <ChannelList link="news" title="News" logo={<TbNewSection />} />
        <ChannelList link="live" title="Live" logo={<MdGroup />} />
        <li
          className={`sm:w-full sm:hidden inline-block group ${
            isDark
              ? "hover:bg-darkBg hover:text-white"
              : "hover:bg-white hover:text-deepBg"
          } px-2 py-[1px] rounded-sm`}
        >
          <Link
            className="flex items-center justify-start gap-2 flex-col sm:flex-row"
            to="/"
          >
            <BsMenuUp />
            <span>Menu</span>
