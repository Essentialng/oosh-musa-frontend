...
const Comment: React.FC<CommentProps> = ({
  ...
  const handleLike = () => {
    // if user already like the comment
    if(likes.includes(user._id)) return toast.error('You already like this post')
    const payload = {
        userId: user._id,
        commentId : id
    }
  ...
  const handleDislike = () => {
    if(!likes.includes(user._id)) return toast.error('You have not like this post')
        const payload = {
            userId: user._id,
            commentId : id
        }
        ...
        <header className="flex items-center gap-2 justify-between">
          ...
          <div className={`dropdown dropdown-end ${isDark ? 'text-deepBg' : 'bg-lightBg'}`}>
            ...
            <ul tabIndex={0} className="dropdown-content menu bg-base-100 rounded-box z-[1] w-32 p-2 shadow">
                {authorId === user?._id ? <li><button onClick={handleDeleteComment}>{loadingDelete ? <LoaderSpinner color={'blue'}/> : 'delete'}</button></li> : null}
                {authorId !== user?._id ? <li><button onClick={()=>{setOpenReport(true)}}>report</button></li> : null}
            </ul>
      </div>
        </header>
