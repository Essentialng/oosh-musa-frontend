  ...
  return (
    <div>
         <section className='w-full h-[300px] relative text-sm'>
            {
                data?.data?.profile ?
                <img className='object-cover h-full w-full' src={data?.data?.profile} alt='background'/>
                :
            ...
            <div className='absolute -bottom-8 left-0 flex items-start justify-start gap-5'>
                {data?.data?.avatar ?
                    <img className='w-10 h-10 rounded-full object-cover shadow-md' src={data?.data?.avatar} alt="profile" />
                    :
                ...
                <div>
                    ...
                    <div className={`flex items-start flex-col shadow-md p-5 justify-start gap-2 font-semibold rounded-md ${isDark ? 'bg-deepBg text-lightText' : 'bg-deepLight text-deepBg'}`}>
                          ...
                          <p className='text-balance'>{data?.data?.followers?.length} <span className='text-blue-400'>followers</span></p>
